...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        test_sq = np.sum(np.square(X), axis=1).reshape(num_test, 1)  # shape (num_test, 1)
        train_sq = np.sum(np.square(self.X_train), axis=1)  # shape (num_train,)

    # Compute matrix product (for the 2xy term in the distance formula)
