...
def softmax_loss(x, y):
    ...
    # Numerical stability fix: subtract max from each score row
...
def batchnorm_backward(dout, cache):
    ...
    x, sample_mean, sample_var, std, gamma, x_hat, shape, axis = cache   

    dbeta = dout.reshape(shape).sum(axis)  # derivative of beta
    dgamma = (dout * x_hat).reshape(shape).sum(axis) 
    dx_hat = dout * gamma                                      
    dstd = -np.sum(dx_hat * (x-sample_mean), axis=0) / (std**2)  
    dvar = 0.5 * dstd / std                                    
    dx1 = dx_hat / std + 2 * (x-sample_mean) * dvar / len(dout)  
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = len(dout) * dx - (dx * x_hat).sum(axis=0) * x_hat - dx.sum(axis=0)
...
def layernorm_backward(dout, cache):
    ...
    dx, dgamma, dbeta = batchnorm_backward(dout.T, cache) # same as batchnorm backprop
    dx = dx.T    
...
def conv_backward_naive(dout, cache):
    ...
    # Pad x and initialize gradients
    ...
    for n in range(N):  # over all images
        for f in range(F):  # over all filters
...
def max_pool_forward_naive(x, pool_param):
    ...
    # Perform max pooling
    for n in range(N):  # over all examples in the batch
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # Select the window to apply the pooling operation
                    x_window = x[n, c, h_start:h_end, w_start:w_end]
                    # Find the maximum value's index within the window
                    max_idx = np.argmax(x_window)
                    # Convert the index to 2D coordinates
                    max_coord = np.unravel_index(max_idx, (pool_height, pool_width))
                    # Map the coordinates to the full x dimensions
                    max_h = h_start + max_coord[0]
                    max_w = w_start + max_coord[1]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # Reshape the input to use the existing batch normalization logic
    x_transposed = x.transpose(0, 2, 3, 1)  # change shape to (N, H, W, C)
    x_flat = x_transposed.reshape(-1, C)  # collapse the first three dimensions
    ...
    # Update bn_param with the new running mean and variance
    bn_param['running_mean'] = running_mean
    bn_param['running_var'] = running_var
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # Reshape dout to fit batch normalization backward function
    dout_reshaped = dout.transpose(0, 2, 3, 1).reshape(-1, C)
    
    # Call the backward batch normalization function
    dx_reshaped, dgamma, dbeta = batchnorm_backward(dout_reshaped, cache)
    
    # Reshape dx back to the original dimensions (N, C, H, W)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_grouped = x_grouped.transpose(0, 1, 3, 4, 2).reshape(N * G, -1)

    
    mean = x_grouped.mean(axis=1, keepdims=True)
    var = x_grouped.var(axis=1, keepdims=True)

   
    x_normalized = (x_grouped - mean) / np.sqrt(var + eps)

    
    x_normalized = x_normalized.reshape(N, G, H, W, C // G).transpose(0, 1, 4, 2, 3).reshape(N, C, H, W)
    out = x_normalized * gamma + beta

    
    cache = (G, x, x_normalized, mean, var, beta, gamma, eps)
