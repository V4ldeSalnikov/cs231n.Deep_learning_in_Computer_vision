...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params['W1'] = weight_scale * np.random.randn(F, C, filter_height, filter_width)
        self.params['b1'] = np.zeros(F)
        
        # Calculate the output size after the convolution and pooling layers
        # Conv layer preserves size, pool layer reduces it by a factor of 2
        H_conv_out = H // 2
        W_conv_out = W // 2
        
        # Initialize weights and biases for the hidden affine layer
        self.params['W2'] = weight_scale * np.random.randn(F * H_conv_out * W_conv_out, hidden_dim)
